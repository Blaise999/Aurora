...
function MintPageContent() {
  ...
  const { useConnectors, connect } = useConnect();
      ...
      const saveMintedNfts = async () => {
        ...
        try {
          const nftImage =
            ...
            resolveImage(selectedNft || {}) ||
            "";
          const nftName =
            selectedNft?.name ||
            `Minted NFT #${confirmResult.tokenIds[0] || ""}`;
          const nftDesc =
            selectedNft?.description ||
            "A unique minted NFT from AuroraNft";

          const liveMintFee = parseFloat(
            selectedNft?.mintFee || settings?.minting_fee || "0.002"
          );
          const liveNftPrice = parseFloat(
            selectedNft?.mintPrice || settings?.minting_fee || "0.002"
          );
          const liveTotalValue = Number(
            (liveNftPrice + liveMintFee).toFixed(6)
          );
          ...
          const inserts = confirmResult.tokenIds.map((tid) => ({
            ...
            nft_name: nftName,
            description: nftDesc,
            collection: selectedNft?.collection || null,
            token_type: selectedNft?.tokenType || "ERC-721",
            attributes: selectedNft?.attributes || [],

            nft_price_eth: liveNftPrice,
            mint_fee_eth: liveMintFee,
            total_value_eth: liveTotalValue,
            pricing_source: "mint",
          }));
          ...
          if (error) {
            ...
            console.log(
              `Saved ${inserts.length} minted NFTs to profile ${profileId}`
            );
          }
        } catch (err) {
  ...
  }, [confirmResult, profileId, selectedNft, targetChainId, savingToDb, settings]);
  ...
  const nftDesc =
    dn?.description ||
    "A unique digital collectible from a curated collection.";
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <nav className="flex items-center gap-2 text-xs text-muted-dim mb-6">
          ...
          <span className="text-text">
            {isSelectedMode ? dn?.name || "Loading..." : "Mint"}
          </span>
        </nav>
              ...
              >
                ...
                {isConnected && (
                  ...
                  >
                    <Heart
                      size={18}
                      className={favorited ? "text-hot fill-hot" : "text-white/70"}
                    />
                  </button>
                )}
              </div>
                ...
                <div className="rounded-card bg-surface2 border border-border-light p-5 sm:p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      {
                        l: "Contract",
                        v: dn.contractAddress
                          ? `${dn.contractAddress.slice(0, 6)}...${dn.contractAddress.slice(-4)}`
                          : "—",
                      },
                ...
                <div className="glass-card rounded-card p-6 sm:p-8 space-y-6">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      {["tx_pending", "tx_signing", "confirming"].includes(state) && (
                        <Badge color="warning" dot>Pending</Badge>
                      )}
                      {state === "tx_success" && (
                        <Badge color="success" dot>Complete</Badge>
                      )}
