...
import {
  LayoutDashboard, Settings, Users, BarChart3, Activity,
  Menu, X, Wallet, ChevronRight, Shield, DollarSign,
  Globe, MessageCircle, Bell
} from 'lucide-react';
...
export default function AdminShell({ children, activeTab = 'overview', onTabChange }) {
  ...
  return (
    <div className="min-h-screen bg-bg flex">
      {sidebarOpen && (
        <div className="fixed inset-0 bg-bg/60 backdrop-blur-sm z-40 lg:hidden" onClick={() => setSidebarOpen(false)} />
      )}
      <aside className={`
      ...
      `}>
        <div className="p-5 border-b border-border-light">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <Badge color="violet" className="ml-auto !text-[10px]">Admin</Badge>
          </Link>
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm transition-all duration-200
                  ${activeTab === item.id ? 'bg-white/[0.06] text-text font-medium' : 'text-muted hover:text-text hover:bg-white/[0.03]'}`}
              >
                ...
                {activeTab === item.id && <ChevronRight size={14} className="ml-auto text-muted-dim" />}
              </button>
            );
          })}
        </nav>
        ...
        <header className="sticky top-0 z-30 bg-bg/80 backdrop-blur-md border-b border-border-light">
          <div className="flex items-center justify-between px-6 py-3.5">
            <div className="flex items-center gap-3">
              <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-1.5 text-muted hover:text-text">
                <Menu size={20} />
              </button>
              <h1 className="font-display font-semibold text-text capitalize">{activeTab === 'pricing' ? 'NFT Pricing' : activeTab === 'visitors' ? 'Visitors & IPs' : activeTab === 'nfts' ? 'NFTs & Sync' : activeTab}</h1>
            </div>
            <div className="flex items-center gap-3">
              ...
              <Badge color="success" dot className="hidden sm:flex">Base</Badge>
